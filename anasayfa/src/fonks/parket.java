...
public class parket {
    ...
    public boolean parket(String parkYeri, String ParkId) {
        String queryCheck = "SELECT kullanici_id FROM yer WHERE kullanici_id = ? AND durum = 2 or durum = 1"; // Kullanıcı zaten rezervasyon yaptı mı kontrolü
        String queryGetParkId = "SELECT otopark_id FROM otopark WHERE otopark_adi = ?";
        String queryUpdate = "UPDATE yer SET durum = 2, kullanici_id = ?, tarih = ? WHERE yer_id = ? AND otopark_id = ?";
